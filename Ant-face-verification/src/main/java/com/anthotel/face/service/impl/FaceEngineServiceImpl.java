...
//        List<FaceUserInfo> faceInfoList = userFaceInfoMapper.getUserFaceInfoByGroupId(groupId); //从数据库中取出人脸库
        List<FaceUserInfo> faceInfoList = null;
     ...
     */
    @Override
    public FaceSimilar comparePeopleIdCard(FaceFeature peopleFeature, FaceFeature idCardFeature) {
        FaceEngine faceEngine = null;
        try {
            //获取引擎对象
            faceEngine = faceEngineObjectPool.borrowObject();
            ...
            faceEngine.compareFaceFeature(peopleFeature, idCardFeature, faceSimilar);
            return faceSimilar;
        } catch (Exception e) {
            logger.error("", e);
        } finally {
